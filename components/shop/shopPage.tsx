...
const [filteredProducts, setFilteredProducts] = useState<Product[]>([]);
  ...
  const renderPaginationButtons = () => {
    ...
    const addEllipsis = (key: string) => (
      <span key={key} className="w-10 h-10 mx-1 flex items-center justify-center text-gray-500">
        ...
      </span>
    );
  ...
  return (
    <div className="container dark:text-white dark:bg-gray-800">
      <div className="row dark:text-white dark:bg-gray-800">
      <ShopFilters
...
/>
        ...
        <div className="col-lg-9">
          <div>
            <p className="text-gray-700 dark:text-gray-400">
              Showing {offset + 1}–{Math.min(offset + limit, totalProducts)} of {totalProducts} results
            </p>
          </div>
          <div className="row dark:text-white dark:bg-gray-800">
  ...
  <p className="text-gray-700 dark:text-gray-400">No products found.</p>
          ...
          <div className="product__pagination flex justify-center mt-4">{renderPaginationButtons()}</div>
        </div>
      </div>
    </div>
  );
